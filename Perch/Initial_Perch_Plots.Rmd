...
```{r o2temp, echo=FALSE,cache=TRUE,results='hide',message="hide",warning="hide"}
...
```
```{r tdo3 plot,echo=FALSE,fig.width=6.5,fig.height=3,fig.align="center"}
...
myplot = ggplot(o2temp.cr,aes(x=sampledate,y=depth,group=1)) +
    geom_rect(aes(xmin = as_date("2012-04-01"), xmax = as_date("2013-12-31"), ymin = -Inf, ymax = Inf), fill = "grey", alpha = 0.03) +
    geom_line()
myplot + theme_bw() + theme(panel.border = element_blank(), 
    panel.grid.major = element_blank(),panel.grid.minor = element_blank(), 
    axis.line = element_line(colour = "black")) + 
    labs(x="Year",y="Depth (m) at DO~3 mg/L") + ylim(c(20,0))
```
Depth at DO~3 mg/L estimated the same way as described for TDO3
\newpage
```{r nutrient data, echo=FALSE,cache=TRUE}
...
```{r nutrient plots,echo=FALSE,fig.width=6.5,fig.height=3,fig.align="center"}
